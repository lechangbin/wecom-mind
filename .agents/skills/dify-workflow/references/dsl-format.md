...
Dify 1.14.2 Code node constraints:

- `code_language` must be `"python3"` or `"javascript"`; do not use `"json"`.
- `outputs` supports only `"string"`, `"number"`, `"object"`, `"boolean"`, `"array[string]"`, `"array[number]"`, `"array[object]"`, and `"array[boolean]"`.
- The object returned by `main()` must contain exactly the declared output keys: no missing keys and no extra keys.
- Do not use generic Dify variable types such as `"integer"`, `"secret"`, `"file"`, `"array"`, `"array[file]"`, `"any"`, or `"array[any]"` in Code node `outputs`.
